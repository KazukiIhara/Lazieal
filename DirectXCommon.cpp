...
cDirectXCommon::cDirectXCommon() {
}

cDirectXCommon::~cDirectXCommon() {
}

void cDirectXCommon::Initialize(bool enableDebugLayer) {
	// FPS固定処理初期化
	InitializeFixFPS();
	// DXGIデバイスの初期化
...
void cDirectXCommon::InitializeDXGIDevice(bool enableDebugLayer) {
#ifdef _DEBUG
	...
	for (UINT i = 0; dxgiFactory_->EnumAdapterByGpuPreference(i,
		...
		if (!(adapterDesc.Flags & DXGI_ADAPTER_FLAG3_SOFTWARE)) {
			// 採用したアダプタの情報をログに出力。
			cLogger::Log(cLogger::ConvertString(std::format(L"Use Adapter : {}\n", adapterDesc.Description)));
			break;
		}
	...
	for (size_t i = 0; i < _countof(featureLevels); ++i) {
		...
		if (SUCCEEDED(hr_)) {
			...
			cLogger::Log(std::format("featureLevel : {}\n", featureLevelStrings[i]));
			break;
		}
	}
	...
	// 初期化完了ログ
	cLogger::Log("Complete Create D3D12DDevice!\n omdetou/(^ v ^)/\n");
...
void cDirectXCommon::CreateSwapChain() {
	swapChainDesc_.Width = cWinAPI::kClientWidth;				// 画面の幅、ウィンドウのクライアント領域を同じものにしておく
	swapChainDesc_.Height = cWinAPI::kClientHeight;				// 画面の高さ、ウィンドウのクライアント領域を同じものしておく
	...
	hr_ = dxgiFactory_->CreateSwapChainForHwnd(commandQueue_.Get(), win_->GetHwnd(), &swapChainDesc_,
...
void cDirectXCommon::CreateDepthStencilView() {
	...
	depthStencilResource_ = CreateDepthStencilTextureResource(device_.Get(), cWinAPI::kClientWidth, cWinAPI::kClientHeight);
...
void cDirectXCommon::SettingViewport() {
	// ビューポートの設定
	D3D12_VIEWPORT viewport{};
	// クライアント領域のサイズと一緒にして画面全体に表示
	viewport.Width = cWinAPI::kClientWidth;
	viewport.Height = cWinAPI::kClientHeight;
	viewport.TopLeftX = 0;
	viewport.TopLeftY = 0;
...
void cDirectXCommon::SettingScissorRect() {
	// シザー矩形の設定
	D3D12_RECT scissorRect{};
	// 基本的にビューポートと同じ矩形が構成されるようにする
	scissorRect.left = 0;
	scissorRect.right = cWinAPI::kClientWidth;
	scissorRect.top = 0;
	scissorRect.bottom = cWinAPI::kClientHeight;

	// シザー矩形を設定
	commandList_->RSSetScissorRects(1, &scissorRect);
}

void cDirectXCommon::InitializeFixFPS() {
	// 現在時間を記録する
	reference_ = std::chrono::steady_clock::now();
}

void cDirectXCommon::UpdateFixFPS() {
	// 1/60秒ぴったりの時間
