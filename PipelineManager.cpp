...
void cPipelineManager::CompileShaders() {
	...
	// Object2Dのシェーダーをコンパイル
	CompileObject2DShaders();
}
...
void cPipelineManager::CreateObject2DRootSignature() {
	HRESULT hr = S_FALSE;
	...
	//バイナリをもとに生成
	rootSignature_[kObject2d] = nullptr;
	hr = directX_->GetDevice()->CreateRootSignature(0, signatureBlob->GetBufferPointer(),
		signatureBlob->GetBufferSize(), IID_PPV_ARGS(&rootSignature_[kObject2d]));
	assert(SUCCEEDED(hr));
}

void cPipelineManager::CreateObject2DGrapchicsPipelineStateObject() {
	HRESULT hr;

	assert(rootSignature_[kObject2d]);
	assert(object2DVertexShaderBlob_);
	assert(object2DPixelShaderBlob_);

	D3D12_GRAPHICS_PIPELINE_STATE_DESC graphicsPipelineStateDesc{};
	graphicsPipelineStateDesc.pRootSignature = rootSignature_[kObject2d].Get();
	graphicsPipelineStateDesc.InputLayout = Object2DInputLayoutSetting();
	graphicsPipelineStateDesc.VS = { object2DVertexShaderBlob_->GetBufferPointer(),
	object2DVertexShaderBlob_->GetBufferSize() };
	graphicsPipelineStateDesc.PS = { object2DPixelShaderBlob_->GetBufferPointer(),
	object2DPixelShaderBlob_->GetBufferSize() };
	graphicsPipelineStateDesc.RasterizerState = Object2DRasterizerStateSetting();
	//書き込むRTVの情報
	graphicsPipelineStateDesc.NumRenderTargets = 1;
	graphicsPipelineStateDesc.RTVFormats[0] = DXGI_FORMAT_R8G8B8A8_UNORM_SRGB;
	//利用するトポロジ(形状)のタイプ、三角形
	...
	graphicsPipelineStateDesc.SampleMask = D3D12_DEFAULT_SAMPLE_MASK;
	/*DepthStencilの設定*/
	graphicsPipelineStateDesc.DepthStencilState = Object2DDepthStecilDescSetting();
	graphicsPipelineStateDesc.DSVFormat = DXGI_FORMAT_D24_UNORM_S8_UINT;

	//実際に生成
	for (uint32_t i = 0; i < kBlendModeNum; i++) {
		graphicsPipelineStateDesc.BlendState = BlendStateSetting(i);
		graphicsPipelineState_[kObject2d][i] = nullptr;
		hr = directX_->GetDevice()->CreateGraphicsPipelineState(&graphicsPipelineStateDesc,
			IID_PPV_ARGS(&graphicsPipelineState_[kObject2d][i]));
		assert(SUCCEEDED(hr));
	}
}

D3D12_DEPTH_STENCIL_DESC cPipelineManager::Object2DDepthStecilDescSetting() {
	/*DepthStencilStateの設定*/
	D3D12_DEPTH_STENCIL_DESC depthStencilDesc{};
	/*Depthの機能を有効化する*/
	depthStencilDesc.DepthEnable = true;
...
D3D12_INPUT_LAYOUT_DESC cPipelineManager::Object2DInputLayoutSetting() {
	// InputLayout
	static D3D12_INPUT_ELEMENT_DESC inputElementDescs[2] = {};
	inputElementDescs[0].SemanticName = "POSITION";
	inputElementDescs[0].SemanticIndex = 0;
...
D3D12_RASTERIZER_DESC cPipelineManager::Object2DRasterizerStateSetting() {
	//RasterizerStateの設定
	D3D12_RASTERIZER_DESC rasterizerDesc_{};
	//裏側(時計回り)を表示しない
